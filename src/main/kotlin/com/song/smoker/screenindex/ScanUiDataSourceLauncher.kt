...
class ScanUiDataSourceLauncher(private val project: Project) {
    ...
    fun start(): Boolean {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, "Scan UI → Data Source", true) {
            override fun run(indicator: ProgressIndicator) {
                ...
                try {
                    val service = koin.koin.get<UiDataSourceScanService>()
                    runBlocking {
                    ...
                    println("[ui-datasource] ERROR ${t.javaClass.name}: ${t.message}")
                    t.printStackTrace()
                } finally {
