package com.song.inspection

import com.song.sarif.Finding

class InspectionSummaryPrinter {
    fun print(context: InspectionRunContext, findings: List<Finding>) {
        println("Module: ${context.module} inspections=${context.inspectionIds.joinToString(",")}")
        if (findings.isEmpty()) {
            println("No findings reported by inspections.")
            return
        }

        val byLevel = findings.groupingBy { it.level ?: "unspecified" }.eachCount()
        val byRule = findings.groupingBy { it.ruleId }.eachCount()
            .entries
            .sortedByDescending { it.value }
            .take(5)
            .joinToString(", ") { "${it.key}=${it.value}" }
        println("By level: ${byLevel.entries.sortedBy { it.key }.joinToString(", ") { "${it.key}=${it.value}" }}")
        println("Top rules: $byRule")
    }
}
